...
export function useMarketplaceClient(options: UseMarketplaceClientOptions = {}) {
  // Memorise the options to prevent unnecessary re-renders
  ...
  // Use ref to track if we're currently initialising to prevent race conditions
  ...
  const initializeClient = useCallback(async (attempt = 1): Promise<void> => {
    ...
    try {
      ...
      setState({
        client: null,
        error: error instanceof Error ? error : new Error('Failed to initialise MarketplaceClient'),
        isLoading: false,
        isInitialized: false,
      });
    } finally {
  ...
  // Memorise the return value to prevent object recreation on every render
  return useMemo(() => ({
    ...state,
